...
install_nodejs() {
  ...
  local code=$(curl "https://nodejs.org/download/release/v10.9.0/node-v10.9.0-linux-x64.tar.gz" -L --silent --fail --retry 5 --retry-max-time 15 -o /tmp/node.tar.gz --write-out "%{http_code}")
  if [ "$code" != "200" ]; then
    echo "Unable to download node: $code" && false
  fi
  tar xzf /tmp/node.tar.gz -C /tmp
  rm -rf $dir/*
  mv /tmp/node-v10.9.0-linux-x64/* $dir
  chmod +x $dir/bin/*
}
